...
from config import OUTPUT_CHANNELS, OUTPUT_SAMPLE_RATE, OUTPUT_SAMPLE_WIDTH
from tts_service import _assemble, _concat, _normalize
...
class TestAssembleEndToEnd:
    """The real encode path: concat -> ambience -> mp3 on disk."""
    ...
    def test_writes_a_playable_mp3(self, tmp_path):
        ...
        _assemble(self.build_parts(), [1600], 50, 35, str(out))
    ...
    def test_runs_with_the_ambience_bed_disabled(self, tmp_path):
        out = tmp_path / "dry.mp3"
        _assemble(self.build_parts(), [], 0, 0, str(out))
        assert out.stat().st_size > 0

    def test_ambience_actually_changes_the_output(self, tmp_path):
        dry, wet = tmp_path / "dry.mp3", tmp_path / "wet.mp3"
        _assemble(self.build_parts(), [1600], 0, 0, str(dry))
        _assemble(self.build_parts(), [1600], 90, 90, str(wet))
        assert dry.read_bytes() != wet.read_bytes()

    def test_refuses_to_encode_an_empty_track(self, tmp_path):
        with pytest.raises(ValueError, match="no audio"):
            _assemble([], [], 50, 35, str(tmp_path / "empty.mp3"))
